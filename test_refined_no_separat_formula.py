...
def test_refined_no_separat_search_with_pmids():
    """Test the refined search formula without separat* with 3 in-scope seed PMIDs"""
    base_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi"
    
    search_query = '((Social Isolation[mh]) OR (Loneliness[mh]) OR (loneliness[tiab]) OR ("social isolation"[tiab]) OR ("social isolat*"[tiab]) OR (alienat*[tiab]) OR (alone*[tiab]) OR (solitude[tiab])) AND ((Smartphone[mh]) OR (Wearable Electronic Devices[mh]) OR (Mobile Applications[mh]) OR (smartphone*[tiab]) OR ("mobile app*"[tiab]) OR ("mobile application*"[tiab]) OR ("wearable device*"[tiab]) OR ("digital phenotyping"[tiab]) OR ("passive sensing"[tiab]) OR ("mobile health"[tiab]) OR (mhealth[tiab]))'
    ...
    try:
        ...
        print("=== REFINED SEARCH FORMULA TEST (NO SEPARAT*) ===")
        ...
        print(f"Noise Reduction: Removed separat* term (958,482 hits)")
        
        return len(captured_pmids) == 3 and total_count <= 3000
        
    except Exception as e:
        print(f"Error: {e}")
        return False

success = test_refined_no_separat_search_with_pmids()
