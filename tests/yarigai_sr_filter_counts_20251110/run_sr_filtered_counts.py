...
Systematic ReviewフィルターをANDで適用したヒット数を取得するスクリプト。
...
OUTPUT_PATH = os.path.join(
    REPO_ROOT,
    "tests",
    "yarigai_sr_filter_counts_20251110",
    "sr_filtered_counts_20251110.md",
)
...
def write_report(all_time: Optional[int], five_year: Optional[int], three_year: Optional[int], combined_query: str, sr_filter_query: str) -> None:
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        ...
        f.write("- templates/SR_filter.md に含まれる Systematic Review フィルター\n\n")
        f.write("## ヒット数\n\n")
        f.write("| 期間 | ヒット数 |\n")
        f.write("|------|----------|\n")
